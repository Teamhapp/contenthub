...
function SuccessContent() {
  ...
  const confettiDots = [
    ...
    { color: "bg-brand-500", size: "w-2 h-2", top: "8%", right: "35%", delay: "0.6s" },
  ...
  return (
    <div className="relative max-w-2xl mx-auto px-4 py-16 sm:py-24 text-center animate-fade-in overflow-hidden">
      ...
      <h1 className="text-4xl font-display font-bold text-surface-900 mb-3">Purchase Complete!</h1>
      <p className="text-surface-500 text-lg mb-6 max-w-md mx-auto">
        Your content is now available in your library. Start enjoying it right away.
      </p>

      {/* Transaction ID Chip */}
      {transactionId && (
        <div className="inline-flex items-center gap-2 bg-surface-100 border border-surface-200 px-4 py-2 rounded-full mb-10">
          <span className="text-[10px] font-semibold uppercase tracking-widest text-surface-400">TX</span>
          <span className="text-xs font-mono text-surface-600 tracking-wide">{transactionId}</span>
        </div>
      )}

      {/* What's Next Section */}
      <div className="mt-4 mb-10">
        <p className="text-xs font-semibold uppercase tracking-widest text-surface-400 mb-5">What&apos;s next?</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
          ...
          >
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-brand-50 to-brand-100 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-300">
              <IconLibrary className="w-6 h-6 text-brand-600" />
            </div>
            <h3 className="font-display font-bold text-surface-900 mb-1">Go to Library</h3>
            <p className="text-xs text-surface-400">Access your purchased content</p>
          </Link>
          <Link
            href="/browse"
            className="card p-6 text-center group hover:shadow-soft-lg transition-all duration-300 hover:-translate-y-0.5"
          >
            ...
            <h3 className="font-display font-bold text-surface-900 mb-1">Browse More</h3>
            <p className="text-xs text-surface-400">Discover more great content</p>
          </Link>
        </div>
      </div>

      {/* Trust Badge */}
      <div className="inline-flex items-center gap-2 text-surface-400 text-xs bg-surface-50 border border-surface-100 rounded-full px-5 py-2.5">
