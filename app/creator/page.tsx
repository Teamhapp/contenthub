  ...
  if (loading) {
    return (
      <div className="space-y-8 animate-fade-in">
        <h1 className="font-['Plus_Jakarta_Sans'] text-2xl font-bold text-2xl">Creator Dashboard</h1>
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-['Plus_Jakarta_Sans'] text-2xl font-bold text-2xl">Creator Dashboard</h1>
          <p className="text-[#797586] mt-1">Track your content performance.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="stat-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold font-['Plus_Jakarta_Sans'] mt-1">{stats.totalContent || 0}</p>
              <p className="text-xs text-[#797586] mt-1">{stats.publishedContent || 0} published</p>
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold font-['Plus_Jakarta_Sans'] mt-1 text-green-600">{formatPrice(stats.totalRevenue || 0)}</p>
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-[#797586]">Total Views</p>
              <p className="text-3xl font-bold font-['Plus_Jakarta_Sans'] mt-1">{stats.totalViews || 0}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="font-['Plus_Jakarta_Sans'] text-2xl font-bold text-lg">Top Performing Content</h2>
          </div>
          {data?.topContent?.length > 0 ? (
            <div className="space-y-1">
              {data.topContent.map((c: any, idx: number) => (
                <div key={c._id} className="flex items-center justify-between py-3 px-3 rounded-lg hover:bg-[#faf8ff] transition-colors">
                  <div className="flex items-center gap-3">
                    <span className="w-6 h-6 rounded-full bg-[#f1f3ff] flex items-center justify-center text-xs font-bold text-[#797586]">{idx + 1}</span>
                    <div>
                      <p className="font-medium text-sm text-[#151b29]">{c.title}</p>
                      <p className="text-xs text-[#797586] capitalize">{c.type} &middot; {c.totalSales} sales</p>
                    </div>
                  </div>
            ...
            <div className="text-center py-8">
              <IconSparkles className="w-8 h-8 text-[#c9c4d7] mx-auto mb-2" />
              <p className="text-[#797586] text-sm">No content yet. Create your first piece!</p>
            </div>
          )}
        </div>
          ...
          {data?.recentSales?.length > 0 ? (
            <div className="space-y-1">
              {data.recentSales.map((sale: any, i: number) => (
                <div key={i} className="flex items-center justify-between py-3 px-3 rounded-lg hover:bg-[#faf8ff] transition-colors">
                  <div>
                    <p className="font-medium text-sm text-[#151b29]">{sale.contentTitle}</p>
