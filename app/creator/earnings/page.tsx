...
export default function EarningsPage() {
  ...
  if (loading) {
    return (
      <div className="space-y-8 animate-fade-in">
        <h1 className="section-title text-2xl">Earnings</h1>
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="section-title text-2xl">Earnings</h1>
        <p className="text-surface-500 mt-1">Track your revenue and sales performance.</p>
      </div>
        ...
        <div className="stat-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold font-display mt-1 text-green-600">{formatPrice(user?.balance || 0)}</p>
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold font-display mt-1">{formatPrice(stats.totalRevenue || 0)}</p>
            </div>
          ...
          <table className="table-premium">
            ...
            <tbody>
              {data.recentSales.map((sale: any, i: number) => (
                <tr key={i}>
                  <td className="font-medium text-surface-900">{sale.contentTitle}</td>
                  <td className="text-surface-600">{formatPrice(sale.price)}</td>
                  <td>
                    <span className="badge-green text-xs font-semibold">{formatPrice(sale.creatorShare)}</span>
                  </td>
                  <td className="text-surface-500">{new Date(sale.date).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
